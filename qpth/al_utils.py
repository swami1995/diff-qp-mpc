...
@torch.jit.script
def warm_start_al(x: torch.Tensor, lamda: torch.Tensor, rho: torch.Tensor, cost_start: torch.Tensor, cost_hist: torch.Tensor, lam_hist: torch.Tensor, rho_hist: torch.Tensor):
    n_batch = x.size(0)
    ...
    lamda = lamda*(lamda_hist.norm(p=2, dim=-1)/lamda.norm(p=2, dim=-1)).unsqueeze(-1)
    rho = rho_hist[cost_sim_idx, batch_idx]
    return lamda, rho

def merit_function(xu, Q, q, dx, x0, lamda, rho, x_lower, x_upper, u_lower, u_upper, diag_cost=True):
    bsz = xu.size(0)
...
def constraint_res_jac1(xu, x0, dx, x_lower, x_upper, u_lower, u_upper):
    ...
    constraint_jac_clamp[:, -n_ineq:] = constraint_jac_clamp[:, -n_ineq:] * mask[...,None] 
...
def constraint_res_jac2(xu, x0, dx_jac, x_lower, x_upper, u_lower, u_upper):
    ...
    res_ineq, res_ineq_clamp, res_ineq_jac, res_ineq_jac_clamp = dyn_res_ineq_jac(x, u, x0, x_lower, x_upper, u_lower, u_upper)
...
def dyn_res_eq(x: torch.Tensor, u: torch.Tensor, dx: torch.nn.Module, x0: torch.Tensor) -> torch.Tensor:
    ...
    bsz, T, xsize = x.size()
    _, _, usize = u.size()
    x_next = dx(x[:,:-1].reshape(-1, xsize), u[:,:-1].reshape(-1, usize)).view(bsz, T-1, xsize)
    res = (x[:,1:,:] - x_next)
...
def dyn_res_eq_jac(x, u, dx_jac, x0):
    ...
    u_size = u.shape[-1]
    x_next, dynamics_jacobian = dx_jac(x[:,:-1].reshape(-1, x_size), u[:,:-1].reshape(-1, u_size))
    # x_next = x[:,1:]
    ...
    return dyn_res_eq_jac_mat_filling(x, dynamics_jacobian, x0, x_next.view(bsz, T-1, x_size), id_x, x_size, u_size, bsz, T)

# @torch.jit.script
def dyn_res_eq_jac_mat_filling(x, dynamics_jacobian, x0, x_next, id_x, x_size : int, u_size : int, bsz : int, T : int):
    x_res = x[:,1:,:] - x_next
...
def dyn_res_ineq_jac(x, u, x0, x_lower, x_upper, u_lower, u_upper):
    bsz, T, x_size = x.shape
    res, res_clamp, id_xu = dyn_res_ineq_jac_jit(x, u, x0, x_lower, x_upper, u_lower, u_upper)
...
@torch.jit.script
def compute_cost_gradient(xu : torch.Tensor, Q : torch.Tensor, q : torch.Tensor, diag_cost : bool = True) -> torch.Tensor:
    C = Q
    c = q
    if diag_cost:
        return C*xu + c
...
class NewtonAL(torch.autograd.Function):
    @staticmethod
    def forward(ctx, meritfn, dyn_fn, cost_fn, merit_grad_hessfn, xi, x0, lam, rho, Q, q, threshold, eps, ls):
        ...
        merit_delta = 1
        while merit_delta > threshold and nstep < 4 and stepsz > 1e-8:# and update_norm > 1e-3*init_update_norm:
            nstep += 1
            ...
            if ls:
                x_est, new_merit, stepsz, status = line_search_newton(update, x_est, meritfnQ, merit)
            else:
                x_est = x_est + update
                new_merit = meritfnQ(x_est)
                
            if x_est.isnan().sum() > 0 or x_est.isinf().sum() > 0:# or new_merit.isnan().sum() > 0 or new_merit.isinf().sum() > 0:
                ipdb.set_trace()
            dyn_res = dyn_fn(x_est)
    ...
    def backward(ctx, x_grad, status_grad):
        ...
        return None, None, None, None, None, None, None, None, Q_grad, q_grad, None, None, None
