...
from qpth import qp_wrapper as mpc
from qpth.qp_wrapper import GradMethods, QuadCost
from qpth.env_dx import pendulum, cartpole, pendulum_th
...
class IL_Env:
    def __init__(self, env, lqr_iter=500, mpc_T=20, slew_rate_penalty=None):
        ...
        elif self.env == 'pendulum_th':
            self.true_dx = pendulum_th.PendulumDx()
    ...
    def sample_xinit(self, n_batch=1):
        ...
        if self.env in ['pendulum', 'pendulum-complex']:
            ...
            th = uniform(n_batch, -(1/2)*np.pi, (1/2)*np.pi)
            thdot = uniform(n_batch, -1., 1.)
            xinit = torch.stack((th, thdot), dim=1)
        elif self.env == 'cartpole':
