...
def main():
    ...
    parser.add_argument("--data_noise_type", type=int, default=0)  # 0: no noise, 1: noise x, 2: noise u, 3: noise both
    parser.add_argument("--data_noise_std", type=float, default=0.01)
    parser.add_argument("--data_noise_mean", type=float, default=0.0)
    parser.add_argument("--test_noise_type", type=int, default=0)  # 0: no noise, 1: noise x, 2: noise u, 3: noise both
    parser.add_argument("--test_noise_std", type=float, default=0.01)
    parser.add_argument("--test_noise_mean", type=float, default=0.0)
    ...
    for i in range(20000):
        ...
        if args.env == "pendulum":
            traj_sample["state"] = unnormalize_states_pendulum(
                traj_sample["state"])
        ...
        if args.deq:
            ...
            # ipdb.set_trace()
            trajs, dyn_res = policy(traj_sample["state"][:, 0], traj_sample["state"], traj_sample["action"],
                                    traj_sample["mask"], iter=i, qp_solve=qp_solve, lastqp_solve=lastqp_solve)
            end = time.time()
