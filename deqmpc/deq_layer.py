...
class DEQLayerEE(DEQLayer):
    ...
    def forward(self, in_obs_dict, in_aux_dict):
        ...
        dx_ref = self.output_layer(z_out)
        # ipdb.set_trace()
        dx_ref = dx_ref.reshape(-1, self.T - 1, 2)
        vel_ref = dx_ref[..., 1:]
        dx_ref = dx_ref[..., :1] * self.dt
        x_ref = torch.cat([dx_ref + x_prev[..., :1, :1], vel_ref], dim=-1)
        # x_ref = torch.cat([x_prev[..., 1:, :self.nq] + dx_ref, vel_ref + x_prev[..., 1:, self.nq:]], dim=-1)
        # x_ref = torch.cat([dx_ref + _obs[:, :, :self.nq], vel_ref], dim=-1)
        _obs2 = torch.stack((_obs[...,6],_obs[...,13]), dim=-1)
        x_ref = torch.cat([_obs2, x_ref], dim=-2)
        u_ref = torch.zeros_like(x_prev[..., :self.nu])
        x_ref_full = torch.zeros_like(x_prev)
        x_ref_full[...,6] = x_ref[..., 0]
        x_ref_full[...,13] = x_ref[..., 1]
        # ipdb.set_trace()
        out_mpc_dict = {"x_t": obs, "x_ref": x_ref_full, "u_ref": u_ref}
        out_aux_dict = {"x": x_ref_full, "u": u_ref, "z": z_out}
        return out_mpc_dict, out_aux_dict
    ...
    # TO BE OVERRIDEN
    def setup_output_layer(self):  
        self.out_dim = 2 * (self.T-1)  # state prediction
        if self.layer_type == "mlp":
            self.out_layer = torch.nn.Sequential(
                torch.nn.Linear(self.hdim, self.out_dim)
            )
            ...
            self.out_layer = torch.nn.Sequential(
                ...
                torch.nn.GroupNorm(self.num_groups, self.hdim),
                torch.nn.Mish(),
                torch.nn.Conv1d(self.hdim, 2, self.kernel_width_out, padding='same'),
            )
            self.gradnorm = GradNormLayer(self.out_dim)
            ###################### Implement GradNormLayer for GCN ######################
            # self.convout = torch.nn.Conv1d(
...
class DEQLayerEE2(DEQLayer):
    ...
    def forward(self, in_obs_dict, in_aux_dict):
        ...
        z_out = self.deq_layer(_input1, z + self.embedding_params[iter][None])
        dx_ref = self.output_layer(z_out)
        dx_ref = dx_ref.reshape(-1, self.T - 1, 4)
        # ipdb.set_trace()
        x_ref = dx_ref + torch.cat([x_prev[..., :1, :3], x_prev[..., :1, 6:7]],dim=-1)
        _obs2 = torch.cat((_obs[...,:3], _obs[...,6:7]), dim=-1)
        x_ref = torch.cat([_obs2, x_ref], dim=-2)
        u_ref = torch.zeros_like(x_prev[..., :self.nu])
        x_ref_full = torch.zeros_like(x_prev)
        x_ref_full[...,:3] = x_ref[..., :3]
        x_ref_full[..., 6] = x_ref[..., 3]
        out_mpc_dict = {"x_t": obs, "x_ref": x_ref_full, "u_ref": u_ref}
        out_aux_dict = {"x": x_ref_full, "u": u_ref, "z": z_out}
        return out_mpc_dict, out_aux_dict
    ...
    # TO BE OVERRIDEN
    def setup_output_layer(self):  
        self.out_dim = 4 * (self.T-1)  # state prediction
        if self.layer_type == "mlp":
            self.out_layer = torch.nn.Sequential(
                torch.nn.Linear(self.hdim, self.out_dim)
            )
            ...
            self.out_layer = torch.nn.Sequential(
                ...
                torch.nn.GroupNorm(self.num_groups, self.hdim),
                torch.nn.Mish(),
                torch.nn.Conv1d(self.hdim, 4, self.kernel_width_out, padding='same'),
            )
            self.gradnorm = GradNormLayer(self.out_dim)
            ###################### Implement GradNormLayer for GCN ######################
            # self.convout = torch.nn.Conv1d(
